...
citiesDict = {line.split(',')[0]:
            {"state":line.split(',')[1],"2000Amt":eval(line.split(',')[2])
            ,"2010Amt":eval(line.split(',')[3].rstrip())}
            for line in file}
file.close()
print(citiesDict)
...
for item in citiesDict.items():
    key = item[0]
    st = "{0:},{1:},{2:},{3:},{4:.2f}\n"\
        .format(key,citiesDict[key]["state"],citiesDict[key]["2000Amt"]
                ,citiesDict[key]["2010Amt"],citiesDict[key]["increaseRate"])
